...
export default function CompaniesPageClient({ companies }: Props) {
  ...
  const filteredCompanies = activeFilter === 'All'
    ...
    : companies.filter((c) => {
        ...
        return c.industry && map[activeFilter]?.some(ind =>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-20">
        {hasCompanies ? (
            ...
            {filteredCompanies.length === 0 && (
              <div className="text-center py-20">
                ...
                <button
                  ...
                  style={{ color: '#2563EB' }}
                >
                  {t('companies.clearFilter')}
                </button>
              </div>
            )}

            <div className="mt-12 flex items-center justify-center gap-4 py-6">
              <span className="text-slate-400 text-sm">{t('companies.dontSee')}</span>
              <Link href="/submit" className="inline-flex items-center gap-1.5 font-semibold text-sm transition-colors" style={{ color: '#2563EB' }}>
                {t('companies.submitReview')}
          ...
          <div className="text-center py-24">
            <div className="w-20 h-20 rounded-2xl flex items-center justify-center mx-auto mb-6" style={{ background: 'linear-gradient(135deg, #EFF6FF, #DBEAFE)' }}>
            ...
            <h2 className="font-display text-2xl text-slate-900 mb-3">No companies yet</h2>
            <p className="text-slate-500 mb-8 max-w-md mx-auto">
              Be the first to share your internship experience and help fellow students make better career decisions.
            </p>
            <Link
              href="/submit"
              className="inline-flex items-center justify-center gap-2 px-7 py-3.5 font-semibold rounded-full text-white text-[15px] transition-all duration-300 hover:scale-[1.03]"
              style={{
                background: 'linear-gradient(135deg, #1B365D, #2563EB)',
                boxShadow: '0 4px 14px -3px rgba(37,99,235,0.35)',
              }}
            >
              {t('companies.emptyCta')}
